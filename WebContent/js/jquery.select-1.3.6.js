...
$.fn.extend({
	sSelect: function() {
		...
		var selecttxtwidth=parseFloat($('#'+selectId+' select').css("left"));//此left用于控制宽度
		//$('#'+selectId+' h4').css({width:selectWidth});选择框内容宽度
		...
		if(selectmaxwidth>selectWidth){
			...
			$('#'+selectId+' ul').css({width:selectmaxwidth-2+'px',display:'none'});
		}else{
		...
		var click_fun =function(){
				...
				$('#'+selectId+' li').click(function(e){
						...
						$('#'+selectId+' h4').empty().append($('#'+selectId+' option:selected').text());
						$('#'+selectId+' h4').attr('title',$('#'+selectId+' option:selected').text());
						if($('#selectyy1__jQSelect0 h4').html()=='选择应用'){ //转化仪表选择后的样式
							$('#selectyy1__jQSelect0 h4').removeClass("h4style")
						}else{
							$('#selectyy1__jQSelect0 h4').addClass("h4style")
						}
						if($('#selectyy2__jQSelect0 h4').html()=='选择应用'){ //转化仪表选择后的样式
	...
	setSelectValue:function(sID){
		...
		$('#'+sID+' ul').html(content);
			//document.getElementById(sID).getElementsByTagName("ul")[0].scrollTop
		...
		var li=document.getElementById(sID).getElementsByTagName("ul")[0].getElementsByTagName("li");
		for(var j=0; j<=li.length; j++){
			if(li[j].innerHTML==document.getElementById(sID).getElementsByTagName("h4")[0].innerHTML){
				document.getElementById(sID).getElementsByTagName("ul")[0].scrollTop=j*23;
			break;
			}
		}
	},
