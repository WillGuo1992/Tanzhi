...
// 		}else{
			if(RateTab == 0){
				loadCompareStayChart();
			}else{
				loadCompareStayConversionChart();
			}
// 		}
	}
	function loadRealtimeSum () {
		...
		var url1 = "http://47.93.123.129:8080/timeDistribution/entirePCRs/"+GroupId+"/"+StartDate+" 0"+"/"+StartDate+" "+thishour;
		console.log(url1);
		$.ajax({
			url:url1,
			dataType:"json",
	...
	function todayRealtimeSumCallback (data) {
		...
		var url2 = "http://47.93.123.129:8080/timeDistribution/entirePCRs/"+GroupId+"/"+oneagostr+" 0"+"/"+oneagostr+" "+thishour;
		console.log(url2);
		$.ajax({
			url:url2,
			dataType:"json",
	...
	function oneagoRealtimeSumCallback (data) {
		...
		var url3 = "http://47.93.123.129:8080/timeDistribution/entirePCRs/"+GroupId+"/"+weekagostr+" 0"+"/"+weekagostr+" "+thishour;
		console.log(url3);
		$.ajax({
			url:url3,
			dataType:"json",
	...
	function weekagoRealtimeSumCallback (data) {
		...
		var url4 = "http://47.93.123.129:8080/timeDistribution/entirePCRs/"+GroupId+"/"+monthagostr+" 0"+"/"+monthagostr+" "+thishour;
		console.log(url4);
		$.ajax({
			url:url4,
			dataType:"json",
	...
	function loadRealtimeStayChart () {
		...
		var url = "http://47.93.123.129:8080/timeDistribution/entirePCRs/"+GroupId+"/"+StartDate;
		$.ajax({
			url:url,
			dataType:"json",
	...
	function realtimeStayChartCallback (data) {
		console.log("loadRealtimeStayChart:"+data);
		var code = data.c;
		if(code == 0){
			var dates = new Array();
	...
	function realtimeConversionChartCallback (data) {
		console.log("loadRealtimeStayConversionChart:"+data);
		var code = data.c;
		if(code == 0){
			var dates = new Array();
			dates.push(StartDate);
	...
	function loadRealtimeCompareStayChart () {
		$("#indoor_chart").html("<img src='../img/big_load.gif' />");
		var url = "http://47.93.123.129:8080/timeDistribution/entirePCRs/"+GroupId+"/"+CompareDate;
		$.ajax({
			url:url,
			dataType:"json",
	...
	function loadStayChart () {
		$("#indoor_chart").html("<img src='../img/big_load.gif' />");
		var url = basePath + "jaxrs/entirety/stay/count/nocompare/chart";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,groupId : GroupId,cxl : cxl
		}
		$.post(url,para,loadStayChartCallBack,"json");
	}
	function loadStayChartCallBack (data) {
		var code = data.code;
		if (code == 0) {
	...
	function loadStayConversionChart () {
		$("#indoor_chart").html("<img src='../img/big_load.gif' />");
		var url = basePath + "jaxrs/entirety/stay/conversion/nocompare/chart";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,groupId : GroupId,cxl : cxl
		}
		$.post(url,para,loadStayConversionChartCallBack,"json");
	}
	function loadStayConversionChartCallBack (data) {
		var code = data.code;
		if (code == 0) {
			var dates = data.dates;
			var datas = data.datas;
	...
	function loadCompareStayChart () {
		$("#indoor_chart").html("<img src='../img/big_load.gif' />");
		var  url = basePath + "jaxrs/entirety/stay/count/compare/chart";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,compareDate : CompareDate,groupId : GroupId,cxl :cxl
		}
		$.post(url,para,loadCompareStayChartCallBack,"json");
	}
	function loadCompareStayChartCallBack (data) {
		var code = data.code;
		if (code == 0) {
			var dates = data.dates;
			var datas = data.datas;
	...
	function loadCompareStayConversionChart () {
		$("#indoor_chart").html("<img src='../img/big_load.gif' />");
		var  url = basePath = "jaxrs/entirety/stay/conversion/compare/chart";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,compareDate : CompareDate,groupId : GroupId,cxl :cxl
		}
		$.post(url,para,loadCompareStayConversionChartCallBack,"json");
	}
	function loadCompareStayConversionChartCallBack (data) {
		var code = data.code;
		if (code == 0) {
			var dates = data.dates;
			var datas = data.datas;
		...
		<div class="maindiv">
			<div id="tab_bar_div">
				<ul class="tab_bar">
					...
					<li><a class="select" href="#">整体停留顾客</a></li>
			...
			<div class="dataDiv">
				<div class="dataDiv_title">
					<div class="dataDiv_title_txt">整体停留</div>
					<div id="count_tab" class="tabs_ select_" style="border-left:solid 1px #BCBABB;margin-top:2px;" onclick="changeConversionTab(0)">停留数</div>
					<div id="rate_tab" class="tabs_" style="margin-top:2px;" onclick="changeConversionTab(1)">转化率</div>
				</div>
