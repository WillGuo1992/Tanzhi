...
// 		}else{
			if(RateTab == 0){
				loadCompareStoreyChart();
			}else{
				loadCompareStoreyConversionChart();
			}
// 		}
	}
	...
	function loadRealtimeStayChart () {
		var url = "http://47.93.123.129:8080/timeDistribution/floor/"+ChosenGroupId+"/"+StartDate;
		//var sturl = "http://zanrenpin.vicp.net:8180/timeDistribution/entirePCRs/"+ChosenGroupId+"/"+StartDate;
		$.ajax({
			url:url,
			dataType:"json",
	...
	function realtimeStatisCallback (data) {
		var code = data.c;
		if(code == 0){
			var stays = data.d.pcs[0];
			var pcs = data.d.pcrs[0];
			$("#this_count").html(stays+"|"+pcs+"%");
		}
	}
	...
	function loadRealtimeCompareStayChart () {
		var url = "http://47.93.123.129:8080/timeDistribution/floor/"+ChosenGroupId+"/"+CompareDate;
		$.ajax({
			url:url,
			dataType:"json",
	...
	function loadStoreyChart () {
		var url = basePath + "jaxrs/storey/stay/count/nocompare/chart";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,groupId : ChosenGroupId,cxl : cxl
		}
		$.post(url,para,loadStoreyChartCallBack,"json");
	}
	function loadStoreyChartCallBack (data) {
		var code = data.code;
		if(code == 0){
	...
	function loadStoreyStatistics () {
		var url = basePath + "jaxrs/storey/stay/statistics";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,groupId : ChosenGroupId,cxl : cxl
		}
		$.post(url,para,loadStoreyStatisticsCallBack,"json");
	}
	function loadStoreyStatisticsCallBack (data) {
		var code = data.code;
		if(code == 0){
	...
	function loadStoreyConversionChart () {
		var url = basePath + "jaxrs/storey/stay/conversion/nocompare/chart";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,groupId : ChosenGroupId,cxl : cxl
		}
		$.post(url,para,loadStoreyConversionChartCallBack,"json");
	}
	function loadStoreyConversionChartCallBack (data) {
		var code = data.code;
		if (code == 0) {
			var dates = data.dates;
			var datas = data.datas;
	...
	function loadCompareStoreyChart () {
		var  url = basePath + "jaxrs/storey/stay/count/compare/chart";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,compareDate : CompareDate,groupId : ChosenGroupId,cxl :cxl
		}
		$.post(url,para,loadCompareStoreyChartCallBack,"json");
	}
	function loadCompareStoreyChartCallBack (data) {
		var code = data.code;
		if (code == 0) {
			var dates = data.dates;
			var datas = data.datas;
	...
	function loadCompareStoreyConversionChart () {
		var  url = basePath + "jaxrs/storey/stay/conversion/compare/chart";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,compareDate : CompareDate,groupId : ChosenGroupId,cxl :cxl
		}
		$.post(url,para,loadCompareStoreyConversionChartCallBack,"json");
	}
	function loadCompareStoreyConversionChartCallBack (data) {
		var code = data.code;
		if (code == 0) {
			var dates = data.dates;
			var datas = data.datas;
		...
		<div class="maindiv">
			<div id="tab_bar_div">
				<ul class="tab_bar">
					...
					<li><a class="select" href="#">楼层停留顾客</a></li>
			...
			<div class="dataDiv">
				<div class="dataDiv_title">
					<div class="dataDiv_title_txt"><span id="title_storey">F1</span>停留</div>
					<div id="count_tab" onclick="changeConversionTab(0)" class="tabs_ select_" style="margin-top:2px;border-left:solid 1px #BCBABB">停留数</div>
