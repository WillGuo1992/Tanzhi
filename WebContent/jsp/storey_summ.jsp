...
<script type="text/javascript">
	...
	function loadStoreys () {
		...
		$.post(url,para,loadStoreysCallBack,"json");
	}
	function loadStoreysCallBack (data) {
		var code = data.code;
		if(code == 0){
...
// 		}else{
			loadCompareStoreySummBar();
			loadCompareStoreyResidenceBar();
// 		}
		...
		if(code == 0){
			...
			if(list != null&&list.length > 0){
				...
				for (var i = 0; i < list.length; i++) {
					...
					for (var j = 0; j < Storeys.length; j++) {
						if(Storeys[j].groupId == list[i].gId){
							nameArray.push(Storeys[j].name);
						}
					}
				}
				doPileBarChart(nameArray,RtgtDatas,RtltDatas,"storey_sum");
				//doPileBarChart(["一楼","二楼"],RtgtDatas,RtltDatas,"storey_sum");
			}
		}
	}
	...
	function realtimeResidenceBarCallback (data) {
		...
		if(code == 0){
			...
			if(list != null&&list.length > 0){
				...
				//doBarChart(["一楼","二楼"],RtAvgDatas,"storey_residence");
			}
		}
	}
	...
	function compareRealtimeSummBarCallback (data) {
		...
		if(code == 0){
			...
			if(list != null&&list.length > 0){
				...
				//doStackPileBarChart(["一楼","二楼"],RtgtDatas,RtltDatas,CRtgtDatas,CRtltDatas,"storey_sum");
			}
		}
	}
	...
	function compareRealtimeResidenceBarCallback (data) {
		...
		if(code == 0){
			...
			if(list != null&&list.length > 0){
				...
				//doCompareBarChart(["一楼","二楼"],RtAvgDatas,CRtAvgDatas,"storey_residence");
			}
		}
	}
	function loadStoreySummBar () {
		var url = basePath + "jaxrs/storey/summ/indoor/nocompare/bar";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,mallId : MallId,cxl : cxl
		}
		$.post(url,para,loadStoreySummBarCallBack,"json");
	}
	function loadStoreySummBarCallBack (data) {
		var code = data.code;
		console.log(code);
		if(code == 0){
			...
			if(storeys.length > 0){
				...
				doPileBarChart(storeyNames,datas,lt5Datas,"storey_sum");
			}else{
				$("#storey_sum").html("未查询到数据");
			}
		}else{
	...
	function loadStoreyResidenceBar () {
		var url = basePath + "jaxrs/storey/summ/residence/nocompare/bar";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,mallId : MallId,cxl : cxl
		}
		$.post(url,para,loadStoreyResidenceBarCallBack,"json");
	}
	function loadStoreyResidenceBarCallBack (data) {
		var code = data.code;
		if(code == 0){
			var storeys = data.storeys;
			var datas = data.datas;
			...
			if(storeys.length > 0){
				...
				doBarChart(storeyNames,datas,"storey_residence","分钟");
			}else{
				$("#storey_residence").html("未查询到数据");
			}
		}else{
	...
	function loadCompareStoreySummBar () {
		var url = basePath + "jaxrs/storey/summ/indoor/compare/bar";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,compareDate : CompareDate,mallId : MallId,cxl : cxl
		}
		$.post(url,para,loadCompareStoreySummBarCallBack,"json");
	}
	function loadCompareStoreySummBarCallBack (data) {
		var code = data.code;
		if(code == 0){
			var storeys = data.storeys;
			var datas = data.datas;
			...
			if(storeys.length > 0){
				...
				doStackPileBarChart(storeyNames,datas,lt5Datas,compareDatas,compareLt5Datas,"storey_sum");
			}else{
				$("#storey_sum").html("未查询到数据");
			}
		}else{
	...
	function loadCompareStoreyResidenceBar () {
		var url = basePath + "jaxrs/storey/summ/residence/compare/bar";
		var cxl = Math.random();
		var para = {
				startDate : StartDate,endDate : EndDate,compareDate : CompareDate,mallId : MallId,cxl : cxl
		}
		$.post(url,para,loadCompareStoreyResidenceBarCallBack,"json");
	}
	function loadCompareStoreyResidenceBarCallBack (data) {
		var code = data.code;
		if(code == 0){
			var storeys = data.storeys;
			var datas = data.datas;
			...
			for (var i = 0; i < compareDatas[i].length; i++) {
			...
			if(storeys.length > 0){
				...
				doCompareBarChart(storeyNames,datas,compareDatas,"storey_residence","分钟");
			}else{
				$("#storey_residence").html("未查询到数据");
			}
		}else{
		...
		<div class="maindiv">
			...
			<div class="dataDiv">
				<div class="dataDiv_title">
					<div class="dataDiv_title_txt">各楼层客流概况</div>
				</div>
